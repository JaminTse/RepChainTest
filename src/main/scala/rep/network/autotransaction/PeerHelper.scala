...
import rep.network.autotransaction.PeerHelper.{TestInvoke, createTransactionByInput}
...
object PeerHelper {
  ...
  case object TestInvoke
  case class TestInvoke2(t: Transaction)

  def createTransactionByInput(nodeName: String, chaincodeId: ChaincodeId,
  ...
  override def receive = {
    ...
    case TickInit =>
      if (SystemProfile.getTranCreateDur > 0)
        scheduler.scheduleOnce(SystemProfile.getTranCreateDur.millis, self, TestInvoke)
    ...
    case TestInvoke =>
      try {
        val si = scala.io.Source.fromFile("api_req/json/transfer_" + "test" + ".json", "UTF-8")
        ...
        sendEvent(EventType.PUBLISH_INFO, mediator, pe.getSysTag, Topic.Transaction, Event.Action.TRANSACTION)
        println("Test Invoke Transaction Id: " + t.id)
        RepLogger.trace(RepLogger.System_Logger, this.getLogMsgPrefix(s"########################CREATED TRANS SUCCESSFULLY BY INPUT VALUE ################"))
        RepLogger.trace(RepLogger.System_Logger, this.getLogMsgPrefix(s"#######CREATED TRANS id =${t.id} ################"))
      } catch {
        case e: RuntimeException => throw e
      }
...
//        val li = try si.mkString finally si.close()
//        val t: Transaction = createTransactionByInput(pe.getSysTag, chaincode, "transfer", Seq(li))
        mediator ! Publish(Topic.Transaction, ti.t)
        sendEvent(EventType.PUBLISH_INFO, mediator, pe.getSysTag, Topic.Transaction, Event.Action.TRANSACTION)
        RepLogger.trace(RepLogger.System_Logger, this.getLogMsgPrefix(s"########################TEST INVOKE 2 CREATED TRANS SUCCESSFULLY BY INPUT VALUE ################"))
        RepLogger.trace(RepLogger.System_Logger, this.getLogMsgPrefix(s"#######TEST INVOKE 2 CREATED TRANS id =${ti.t.id} ################"))
      } catch {
        case e: RuntimeException => throw e
      }
  }
